...
export const AnimatedTestimonials = ({ testimonials, autoplay = false }) => {
  ...
  const [rotateY, setRotateY] = useState(0) // State untuk menyimpan nilai rotasi Y

  // Menangani auto-play jika diaktifkan
  useEffect(() => {
    if (autoplay) {
  ...
  // Mengatur rotasi acak hanya di sisi klien
  useEffect(() => {
    setRotateY(Math.floor(Math.random() * 21) - 10) // set random rotateY
  }, [])

  // Fungsi untuk pindah ke testimonial berikutnya
  const handleNext = () => {
    setActive((prev) => (prev + 1) % testimonials.length)
  }

  // Fungsi untuk pindah ke testimonial sebelumnya
  const handlePrev = () => {
    setActive((prev) => (prev - 1 + testimonials.length) % testimonials.length)
  }

  // Mengecek apakah testimonial aktif
              ...
              {testimonials.map((testimonial, index) => (
                <motion.div
                  ...
                  initial={{
                    ...
                    scale: 0.9,
                    z: -100,
                    rotate: rotateY, // Menggunakan nilai rotateY yang di-set di useEffect
                  }}
                  animate={{
                    opacity: isActive(index) ? 1 : 0.7,
                    scale: isActive(index) ? 1 : 0.95,
                    z: isActive(index) ? 0 : -100,
                    rotate: isActive(index) ? 0 : rotateY, // Menggunakan rotasi yang di-set
                  ...
                  exit={{
                    ...
                    scale: 0.9,
                    z: 100,
                    rotate: rotateY, // Sama seperti sebelumnya
                  }}
                  transition={{
                    duration: 0.4,
                    ease: "easeInOut",
